...
$('.gdc-page-tools.email').on('click', function() {
	location.href = 'mailto:?subject='+location.href;
});
$('a[href=#main-content], a.backToTop').on('click',function() {
...
(function() {
$(function($) { 
	var $anim_type_slide = true;
	var $anim_speed = 100;
	var $menu_behavior_click = false;
	var $mouseout_duration = 100;
	var showTimeout;
	
	var isiPad = ("ontouchstart" in document.documentElement) || (navigator.userAgent.match(/iPad/i) != null);//= navigator.userAgent.match(/iPad/i) != null;

if (isiPad) {

	$('.wfui-hover-fat-menu').on('dblclick', '> li > a', function(e) {
	  	window.location.href = e.target.href;
	});

	var $prev_menu;
	$('div.submenu').on('click touchend', 'a', function(e) {
	...
	$('.wfui-hover-fat-menu').on('click touchend', '> li > a', function(e) {

        if ($(e.currentTarget).siblings("div.submenu").size()==0) {
	  		window.location.href = e.target.href;
	    }
		...
		if($(this)[0].className.indexOf("dd-link") <= -1){
			window.location.href = e.target.href;
		}

		var $inst = $(this).closest('.wfui-hover-fat-menu');
		var $animating = $inst.find('.submenu:visible:animated');
		var $visible = $inst.find('.submenu:visible');

		// 1, menu is open and is animating closed
		if (!$animating.length && !$visible.length) {
			$submenu.fadeIn().closest(".dd").addClass("open");
		} else if ($visible.length && $(this)[0] == $prev_menu[0]) {
			window.location.href = e.target.href;
		}
		else if ($visible.length && $(this)[0] != $prev_menu[0]){
			$visible.hide();
			$submenu.show().closest(".dd").addClass("open");
		}

		$prev_menu = $(this);

		clearTimeout($inst.data('hideTimeout'));
	});

	$('.wfui-hover-fat-menu').on('hover', '> li > a', function(e) {});

} else if ($menu_behavior_click) {		//user

	$('.wfui-hover-fat-menu').on('dblclick', '> li > a', function(e) {
	  	window.location.href = e.target.href;
	});

	$('.wfui-hover-fat-menu').on('click', '> li > a', function(e) {
        if ($(e.currentTarget).siblings("div.submenu").size()==0) {
	  		window.location.href = e.target.href;
	    }
		e.preventDefault();
		...
		// 1, menu is open and is animating closed
		if (!$animating.length && !$visible.length) {
			if (isiPad || !$anim_type_slide) {
				$submenu.fadeIn().closest(".dd").addClass("open");
			} else {
				$submenu.slideDown().closest(".dd").addClass("open");
			}
	...
	$('.wfui-hover-fat-menu').on('hover', '> li > a', function(e) {
		...
		if ($animating.length) {
			...
			$animating.stop();
			$animating.hide();
			$submenu.show().height($animating.height()).animate({
				height: WFUIJS.getAutoHeight($submenu),
			}, $anim_speed);
			}
		} 
		else if ($visible.length) {

				// Switch immediately, animate height difference only
			$visible.hide();
			$submenu.show().height($visible.height()).animate({
			height: WFUIJS.getAutoHeight($submenu),
			});
  			$submenu.closest(".dd").addClass("active");
  		 

		} else { 	// 3, menu is not open

			showTimeout = setTimeout(function(){
				if ($inst.find(".dd.open").size()>0) {
					$submenu.fadeIn().closest(".dd").addClass("open");
				}
			}, 300);
		}
...
if(!isiPad){
	$('.wfui-hover-fat-menu').on('mouseover', 'li .submenu', function() {
		clearTimeout($(this).closest('.wfui-hover-fat-menu').data('hideTimeout'));
		$(this).closest(".dd").addClass("active");
	});

	$('.wfui-hover-fat-menu').on('mouseout', '> li > a, li .submenu', function() {
		clearTimeout(showTimeout);
		var $submenu = $(this).siblings('div.submenu');
		var $inst = $(this).closest('.wfui-hover-fat-menu');
		var $visible = $inst.find('.submenu:visible');
	   // if ($visible.length) {
	    //	if ($inst.find(".dd.open").size()>0) {
		//		$('li .submenu:visible', $inst).fadeOut();
		//	} 
	//	} else {
			$inst.data('hideTimeout', setTimeout(function() {
				$('li .submenu:visible', $inst).slideUp();
				$inst.find(".dd.open").removeClass("open");
			}, $mouseout_duration));
...
$('a.submenu-close').click(function() {
	var $inst = $(this).closest('.wfui-hover-fat-menu');
    $inst.closest(".dd").removeClass("active");
    $inst.siblings('div.submenu').slideUp();
});
WFUIJS.getAutoHeight = function($el) {
	var heightOrig = $el.height();
	var autoHeight = $el.css({height: 'auto'}).height();
	$el.height(heightOrig);
	return autoHeight;
}
